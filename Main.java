    ...
    public static boolean performAction(ServiceCenter serviceCenter, int _option, Scanner inputScan){
        ...
        switch(_option){
            case 1:
                ...
                while(true) 
                {
                    try {
                        ...
                        while (level<0 || level>5) { //while level is less than 0 or >=6 (outside 1-5 range)
                            System.out.println("please enter correct number between 1-5");  //asked  to re-enter triage
                            level = inputScan.nextInt();
                        }
                        triageLevel = Integer.toString(level); //update triagelevel with new int input (however triagelevl is string, so must convert int to string)
                        break;
                    }
                    catch(Exception e) { //catches mismatch data type (if a anything but a number is passed as triage level)
                        System.out.println("Please enter a number");
                        triageLevel = inputScan.nextLine();      
                    }    
                }
                inputScan.nextLine(); //buffer the carraige return
            ...
            case 3:
                ...
                inputScan.nextLine(); //buffer carraige return to stop immediate continue
                waitInput(inputScan);
                break;
            case 4:
                clearScreen();
